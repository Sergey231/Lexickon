...
Зависимости направлены от слоя композиции к feature-коду. Domain не знает о
конкретных реализациях репозиториев, SwiftUI и DI-инфраструктуре.
...
распределяет их по feature-фабрикам. Полный набор `AppUseCases` остаётся внутри
...
обратиться к несвязанной части приложения.
...
- Feature не получает `AppContainer` целиком.
...
- При росте feature его фабрику можно вынести в отдельный файл или модуль, не
